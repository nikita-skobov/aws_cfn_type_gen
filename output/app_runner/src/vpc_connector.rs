...
impl cfn_resources::CfnResource for CfnVpcConnector {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if the_val.len() > 40 as _ {
            return Err(format!("Max validation failed on field 'vpc_connector_name'. {} is greater than 40", the_val.len()));
        }
        ...
        if the_val.len() < 4 as _ {
            return Err(format!("Min validation failed on field 'vpc_connector_name'. {} is less than 4", the_val.len()));
        }
