    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(the_val) = &self.security_group_ids {
            if the_val.len() > 50 as _ {
                return Err(format!(
                    "Max validation failed on field 'security_group_ids'. {} is greater than 50",
                    the_val.len()
                ));
            }
        }
        ...
        if let Some(the_val) = &self.storage_capacity {
            if *the_val < 0 as _ {
                return Err(format!(
                    "Min validation failed on field 'storage_capacity'. {} is less than 0",
                    the_val
                ));
            }
        }
        ...
        if the_val.len() > 50 as _ {
            return Err(format!(
                "Max validation failed on field 'subnet_ids'. {} is greater than 50",
                the_val.len()
            ));
        }
...
impl cfn_resources::CfnResource for OntapConfiguration {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(the_val) = &self.route_table_ids {
            if the_val.len() > 50 as _ {
                return Err(format!(
                    "Max validation failed on field 'route_table_ids'. {} is greater than 50",
                    the_val.len()
                ));
            }
        }
        ...
        if let Some(the_val) = &self.throughput_capacity {
            if *the_val < 8 as _ {
                return Err(format!(
                    "Min validation failed on field 'throughput_capacity'. {} is less than 8",
                    the_val
                ));
            }
        }
...
impl cfn_resources::CfnResource for WindowsConfiguration {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(the_val) = &self.aliases {
            if the_val.len() > 50 as _ {
                return Err(format!(
                    "Max validation failed on field 'aliases'. {} is greater than 50",
                    the_val.len()
                ));
            }
        }
        ...
        if *the_val > 100000 as _ {
            return Err(format!(
                "Max validation failed on field 'throughput_capacity'. {} is greater than 100000",
                the_val
            ));
        }
