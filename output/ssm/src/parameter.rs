...
impl cfn_resources::CfnResource for CfnParameter {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if the_val.len() > 1024 as _ {
            return Err(format!("Max validation failed on field 'allowed_pattern'. {} is greater than 1024", the_val.len()));
        }
        ...
        if the_val.len() < 0 as _ {
            return Err(format!("Min validation failed on field 'allowed_pattern'. {} is less than 0", the_val.len()));
        }
        ...
        if the_val.len() > 128 as _ {
            return Err(format!("Max validation failed on field 'data_type'. {} is greater than 128", the_val.len()));
        }
        ...
        if the_val.len() < 0 as _ {
            return Err(format!("Min validation failed on field 'data_type'. {} is less than 0", the_val.len()));
        }
