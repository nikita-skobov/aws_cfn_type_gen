...
impl cfn_resources::CfnResource for CfnWebACLAssociation {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if the_val.len() > 2048 as _ {
            return Err(format!("Max validation failed on field 'web_aclarn'. {} is greater than 2048", the_val.len()));
        }
        ...
        if the_val.len() < 20 as _ {
            return Err(format!("Min validation failed on field 'web_aclarn'. {} is less than 20", the_val.len()));
        }
