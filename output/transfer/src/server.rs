    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(the_val) = &self.protocols {
            if the_val.len() > 4 as _ {
                return Err(format!(
                    "Max validation failed on field 'protocols'. {} is greater than 4",
                    the_val.len()
                ));
            }
        }
...
impl cfn_resources::CfnResource for ProtocolDetails {
    ...
    fn validate(&self) -> Result<(), String> {
        if let Some(the_val) = &self.as2_transports {
            if the_val.len() > 1 as _ {
                return Err(format!(
                    "Max validation failed on field 'as2_transports'. {} is greater than 1",
                    the_val.len()
                ));
            }
        }
...
impl cfn_resources::CfnResource for WorkflowDetails {
    ...
    fn validate(&self) -> Result<(), String> {
        if let Some(the_val) = &self.on_partial_upload {
            if the_val.len() > 1 as _ {
                return Err(format!(
                    "Max validation failed on field 'on_partial_upload'. {} is greater than 1",
                    the_val.len()
                ));
            }
        }
        ...
        if let Some(the_val) = &self.on_upload {
            if the_val.len() > 1 as _ {
                return Err(format!(
                    "Max validation failed on field 'on_upload'. {} is greater than 1",
                    the_val.len()
                ));
            }
        }
